...
export const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (CurrentUser) {
                ...
                const userType = userEmail.startsWith('7') ? 'Supervisor' : userEmail.startsWith('2') ? 'Student' : '';
    ...
    return (
        <div className={`flex items-center justify-center min-h-screen bg-[linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)),url('../../src/assets/images/uj_login.jpg')] bg-cover bg-center bg-no-repeat dark:bg-gray-900 ${isPortrait ? 'portrait' : 'landscape'}`}>
            {isDesktopOrLaptop && (
                <div className="absolute top-10 left-10 flex items-center space-x-4">
                    <img src={Logo} alt="University of Johannesburg Logo" className="w-24 h-24" />
                    <div>
                        <h1 className="text-3xl font-bold text-white dark:text-gray-200">PostGrade Portal</h1>
                        <p className="text-white dark:text-gray-400">University Of Johannesburg</p>
                    </div>
                </div>
            )}
...
//         // if student number starts with 220, its student. If it starts with 210, its lecturer
...
//             <div className={`relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 ${isTabletOrMobile ? 'm-4' : ''}`}>
//                 <div className="flex justify-center rounded-xl h-20 -mt-16 mb-6 p-4 bg-[#FF8503] dark:bg-gray-700">
//                     <h1 className="text-3xl font-extrabold text-white text-center">Login</h1>
//                 </div>
