...
app.post("/request-meeting", async (req, res) => {
  ...
  try {
    ...
    const supervisorInfo = await sendMail(supervisorId, studentId, subject, supervisorHtml);
...
app.post("/confirm-meeting", async (req, res) => {
  ...
  try {
    ...
    const studentInfo = await sendMail(supervisorId, studentId, subject, studentHtml);
